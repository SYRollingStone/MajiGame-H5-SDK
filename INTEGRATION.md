...
**iframe 嵌入场景：自动请求父窗口协助拦截（v0.2.0+）**

如果你的游戏被父站点 iframe 嵌入（如 majigame.com 的 portal 页），按 F12 时焦点通常在父窗口而非 iframe，导致 SDK 内的 keydown 监听根本收不到事件。

从 v0.2.0 起，SDK 启动时如果检测到 `window.self !== window.top` 且 `shortcuts.enabled === true`，会自动向父窗口 `postMessage` 一条注册消息：

```js
{
  type: 'maji-sdk:install-shortcut-blocker',
  version: 1,
  blockKeys: <opts.blockKeys>,
  blockContextMenu: <opts.blockContextMenu>,
}
```

`destroy()` 时会发对应的 `maji-sdk:uninstall-shortcut-blocker` 消息。

**作为接入方游戏**：无需配置，行为自动启用。父窗口不监听这条消息也无害（postMessage 发出去就忘）。

**作为父站点（portal）**：需要在嵌入游戏的容器组件里加一段桥接监听器，否则 F12 拦截不到。参考实现：

```tsx
useEffect(() => {
  let blocking = { blockKeys: false, blockContextMenu: false }

  const isBlockedKey = (e) => {
    if (e.key === 'F12' || e.keyCode === 123) return true
    const m = e.ctrlKey || e.metaKey
    if (m && e.shiftKey && /^[ijc]$/i.test(e.key)) return true
    if (m && !e.shiftKey && /^[us]$/i.test(e.key)) return true
    return false
  }

  const onKeydown = (e) => { if (blocking.blockKeys && isBlockedKey(e)) e.preventDefault() }
  const onContextMenu = (e) => { if (blocking.blockContextMenu) e.preventDefault() }
  const onMessage = (e) => {
    if (e.source !== iframeRef.current?.contentWindow) return  // only our iframe
    const d = e.data
    if (d?.type === 'maji-sdk:install-shortcut-blocker') blocking = { blockKeys: !!d.blockKeys, blockContextMenu: !!d.blockContextMenu }
    if (d?.type === 'maji-sdk:uninstall-shortcut-blocker') blocking = { blockKeys: false, blockContextMenu: false }
  }
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `type` | string | `'maji-sdk:install-shortcut-blocker'` 或 `'maji-sdk:uninstall-shortcut-blocker'` |
| `version` | number | 协议版本，当前 `1`。未来字段变化会升 |
| `blockKeys` | boolean | 是否拦截 F12 等键盘快捷键 |
| `blockContextMenu` | boolean | 是否拦截右键菜单 |

> 安全性：父窗口监听器必须用 `event.source === iframeRef.current?.contentWindow` 校验消息来源，避免第三方 iframe（如广告）冒充游戏发送拦截请求。
...
| iframe 嵌入下 F12 还能开 DevTools | 父窗口没装桥接监听器 | 在父站点容器组件里加上 [5.2 节末尾](#52-securityshortcuts--快捷键--右键拦截) 的接入代码 |
| iframe 嵌入下反调试遮罩误报 | `size` 检测器在 iframe 里 outerWidth/innerWidth 对不上 | 把 `'size'` 从 `detectors` 数组里去掉，例：`detectors: ['debugger', 'timing', 'console', 'toString']` |
